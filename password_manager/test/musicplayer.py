...
def gui_main():
    ...
    scale = Scale(screen, from_=0, to=100, orient=HORIZONTAL, length=200, bg="lightblue", fg="darkblue", troughcolor="royalblue", command=lambda val: set_volume(val))
    scale.set(50)  # Set initial volume to 50%
    scale.grid(row=9, column=0)
    def on_submit():
    ...
    def set_volume(val):
        pg.mixer.music.set_volume(float(val) / 100)
        
    def next():
        if  user_choice.get().strip() == "":
            user_choice.set('1')  
            play_music(get_current_music())
            return
    
        current_music_no = user_choice.get()
        if current_music_no  is None or not current_music_no.isdigit():
            current_music_no = "1"
